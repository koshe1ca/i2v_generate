...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("I2V Generate Desktop - Path B")
    ...
    def _build_generate_tab(self):
        ...
        self.base_model_edit = QLineEdit(); self.motion_adapter_edit = QLineEdit()
        self.prompt_edit = QTextEdit(); self.prompt_edit.setFixedHeight(100)
        ...
        self.frames_spin = QSpinBox(); self.frames_spin.setRange(4, 480)
        ...
        self.photo_preserve_cb = QCheckBox("Photo-preserve mode (Path B)")
        self.keep_bg_cb = QCheckBox("Keep original background")
        ...
        self.overlap_frames_spin = QSpinBox(); self.overlap_frames_spin.setRange(1, 16)
        self.ip_scale_spin = QDoubleSpinBox(); self.ip_scale_spin.setRange(0, 1.5); self.ip_scale_spin.setSingleStep(0.05)
        self.motion_strength_spin = QDoubleSpinBox(); self.motion_strength_spin.setRange(0, 1.5); self.motion_strength_spin.setSingleStep(0.05)
        ...
        adv_form.addRow(self.photo_preserve_cb)
        adv_form.addRow(self.keep_bg_cb)
        ...
        adv_form.addRow("IP-Adapter scale", self.ip_scale_spin)
        adv_form.addRow("Motion strength", self.motion_strength_spin)
        ...
        btn_row.addWidget(self.start_btn); btn_row.addWidget(self.cancel_btn)
        ...
        right_layout.addWidget(self.stage_label); right_layout.addWidget(self.progress_bar); right_layout.addWidget(self.monitor_label)
        ...
        for lbl in [self.preview_first, self.preview_middle, self.preview_last]:
            lbl.setFixedSize(320, 240); lbl.setAlignment(Qt.AlignCenter); lbl.setStyleSheet("border:1px solid #555; background:#222;")
        prev_layout.addWidget(QLabel("First"), 0, 0); prev_layout.addWidget(QLabel("Middle"), 0, 1); prev_layout.addWidget(QLabel("Last"), 0, 2)
        prev_layout.addWidget(self.preview_first, 1, 0); prev_layout.addWidget(self.preview_middle, 1, 1); prev_layout.addWidget(self.preview_last, 1, 2)
        right_layout.addWidget(prev_group)
        self.selected_loras = QListWidget(); right_layout.addWidget(QLabel("Active LoRAs")); right_layout.addWidget(self.selected_loras)
        layout.addWidget(left, 2); layout.addWidget(right, 3)
    ...
    def _build_history_tab(self):
        ...
        layout.addLayout(row); layout.addWidget(self.history_list)
        self.repeat_btn.clicked.connect(self.repeat_from_history); self.open_video_btn.clicked.connect(self.open_selected_history_video)
    ...
    def _build_models_tab(self):
        ...
        self.use_lora_btn = QPushButton("Activate selected LoRA"); self.remove_lora_btn = QPushButton("Remove selected active LoRA")
        ...
        self.refresh_models_btn.clicked.connect(self.refresh_model_library); self.refresh_lora_btn.clicked.connect(self.refresh_lora_library)
        self.add_lora_btn.clicked.connect(self.add_lora_file); self.use_lora_btn.clicked.connect(self.activate_selected_lora)
        self.remove_lora_btn.clicked.connect(self.remove_active_lora); self.add_model_btn.clicked.connect(self.add_model_path)

    def _build_logs_tab(self):
        layout = QVBoxLayout(self.logs_tab)
        self.logs_view = QPlainTextEdit(); self.logs_view.setReadOnly(True)
        self.refresh_logs_btn = QPushButton("Refresh logs")
        layout.addWidget(self.refresh_logs_btn); layout.addWidget(self.logs_view)
    ...
    def _load_settings_into_ui(self):
        s = self.settings
        self.mode_box.setCurrentText(s.mode); self.preset_box.setCurrentText(s.quality_preset)
        self.base_model_edit.setText(s.base_model); self.motion_adapter_edit.setText(s.motion_adapter)
        self.prompt_edit.setPlainText(s.prompt); self.negative_prompt_edit.setPlainText(s.negative_prompt)
        self.input_image_edit.setText(s.input_image or ""); self.ref_video_edit.setText(s.ref_video or "")
        self.output_dir_edit.setText(s.output_override_dir or str(self.output_dir))
        self.frames_spin.setValue(s.video.num_frames); self.steps_spin.setValue(s.video.steps); self.guidance_spin.setValue(s.video.guidance)
        self.width_spin.setValue(s.video.width); self.height_spin.setValue(s.video.height); self.fps_spin.setValue(s.video.fps); self.seed_spin.setValue(s.video.seed)
        self.temporal_cb.setChecked(s.temporal.enable); self.face_restore_cb.setChecked(s.face_restore.enable)
        self.controlnet_cb.setChecked(s.controlnet.enable); self.save_frames_cb.setChecked(s.video.save_frames)
        self.face_lock_cb.setChecked(s.temporal.face_lock_only); self.temporal_strength.setValue(s.temporal.strength)
        self.control_strength.setValue(s.controlnet.conditioning_scale)
        self.photo_preserve_cb.setChecked(s.photo_preserve.enabled); self.keep_bg_cb.setChecked(s.photo_preserve.keep_background)
        self.long_video_cb.setChecked(s.long_video.enabled); self.target_duration_spin.setValue(s.long_video.target_duration_sec)
        self.chunk_frames_spin.setValue(s.long_video.chunk_frames); self.overlap_frames_spin.setValue(s.long_video.overlap_frames)
        self.ip_scale_spin.setValue(s.ip_adapter.scale); self.motion_strength_spin.setValue(s.photo_preserve.motion_strength)
        self._refresh_active_lora_widget(); self.refresh_logs()

    def _collect_settings(self) -> AppSettings:
        s = self.settings
        s.mode = self.mode_box.currentText(); s.base_model = self.base_model_edit.text().strip(); s.motion_adapter = self.motion_adapter_edit.text().strip()
        s.prompt = self.prompt_edit.toPlainText().strip(); s.negative_prompt = self.negative_prompt_edit.toPlainText().strip()
        s.input_image = self.input_image_edit.text().strip() or None; s.ref_video = self.ref_video_edit.text().strip() or None
        s.output_override_dir = self.output_dir_edit.text().strip() or str(self.output_dir)
        s.video.num_frames = self.frames_spin.value(); s.video.steps = self.steps_spin.value(); s.video.guidance = self.guidance_spin.value()
        s.video.width = self.width_spin.value(); s.video.height = self.height_spin.value(); s.video.fps = self.fps_spin.value(); s.video.seed = self.seed_spin.value()
        s.temporal.enable = self.temporal_cb.isChecked(); s.temporal.face_lock_only = self.face_lock_cb.isChecked(); s.temporal.strength = self.temporal_strength.value()
        s.face_restore.enable = self.face_restore_cb.isChecked(); s.controlnet.enable = self.controlnet_cb.isChecked(); s.controlnet.conditioning_scale = self.control_strength.value()
        s.video.save_frames = self.save_frames_cb.isChecked(); s.quality_preset = self.preset_box.currentText(); s.photo_preserve.enabled = self.photo_preserve_cb.isChecked()
        s.photo_preserve.keep_background = self.keep_bg_cb.isChecked(); s.long_video.enabled = self.long_video_cb.isChecked(); s.long_video.target_duration_sec = self.target_duration_spin.value()
        s.long_video.chunk_frames = self.chunk_frames_spin.value(); s.long_video.overlap_frames = self.overlap_frames_spin.value(); s.ip_adapter.scale = self.ip_scale_spin.value(); s.photo_preserve.motion_strength = self.motion_strength_spin.value()
        return s

    def start_generation(self):
        self.settings = self._collect_settings()
        self.stage_label.setText("Starting..."); self.progress_bar.setValue(0)
    ...
    def repeat_from_history(self):
        ...
        self._load_settings_into_ui(); self.tabs.setCurrentWidget(self.generate_tab)
    ...
    def _on_progress(self, stage: str, value: int):
        ...
        denom = max(1, self.steps_spin.value())
        if stage in {"Generating", "Photo-preserve animation"}:
            self.progress_bar.setValue(min(100, int((value / denom) * 100)))
        elif stage == "Done":
    ...
    def _set_preview(self, label: QLabel, path: str):
        pix = QPixmap(path)
        if pix.isNull():
            label.setText("No preview"); return
        label.setPixmap(pix.scaled(label.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))

    def _on_preview(self, p1: str, p2: str, p3: str):
        self._set_preview(self.preview_first, p1); self._set_preview(self.preview_middle, p2); self._set_preview(self.preview_last, p3)

    def _on_finished(self, path: str):
        self.start_btn.setEnabled(True); self.cancel_btn.setEnabled(False); self.refresh_history(); self.refresh_logs()
        QMessageBox.information(self, "Done", f"Video saved:\n{path}")

    def _on_failed(self, message: str):
        self.start_btn.setEnabled(True); self.cancel_btn.setEnabled(False); self.refresh_logs()
